...
int main() {
  ...
  printf("SIT\'s policy on copying does not allow the students to copy source "
         "code as well as assessment solutions from another person AI or other "
         "places. It is the student responsibility to guarantee that their "
         "assessment solutions are their own work. Meanwhile, the students "
         "must also ensure that their work is not accessible by others. Where "
         "such plagiarism is detected, both of the assessments involved will "
         "receive ZERO mark.\n");
  printf("We hereby declare that:\n");
  printf("• We fully understand and agree to the above mentioned plagiarism "
         "policy.\n");
  printf("• We did not copy any code from others or from other places.\n");
  printf("• We did not share our codes with others or upload to any other "
         "places for public access and will not do that in the future.\n");
  ...
  printf("• We agree that we will not disclose any information or material of "
         "the group project to others or upload to any other places for public "
         "access.\n");
  printf("• We agree that we did not copy any code directly from AI generated "
         "sources\n");
  ...
  do {
    ...
    printf("==================================\n");
    printf("  OPEN                                        - Open the database "
           "file & read in all the records\n");
    printf(
        "  SHOW ALL                                    - Show all records\n");
    printf("  INSERT ID=<id>                              - Insert a new "
           "record\n");
    printf("  QUERY ID=<id>                               - Query a record by "
           "ID\n");
    printf("  UPDATE ID=<id> \"<field>=<value>\" \"<...>\"    - Update a "
           "record by ID\n");
    printf("  DELETE ID=<id>                              - Delete a record by "
           "ID\n");
    printf(
        "  SORT                                        - Sort the records\n");
    printf("  SUMMARY                                     - Summary of the "
           "records\n");
    printf("  ADD COLUMN Name=<value>                     - Unique Feature: "
           "Adds new column\n");
    printf("  SAVE                                        - Save the database "
           "file\n");
    printf(
    ...
    sscanf(
        command, "%19s",
        commandPrefix); // Store the first word of the command in commandPrefix
    ...
    if (strcmp(commandPrefix, "OPEN") == 0) {
      ...
      if (!databaseLoaded) {
        printf(
            "CMS: Database is not loaded. Please open the database first.\n");
      } else {
        // Save all the current records into the database file
        save_operation(DB_FILENAME, db, recordCount, custom_column,
                       num_custom_cols);
      }
    } else if (strcmp(commandPrefix, "SORT") == 0) {
      // Show summary of Total number of students, Average mark, Highest and
      // lowest mark
      sort_operation();
