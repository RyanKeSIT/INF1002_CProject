#ifndef CMS_H // include guards to prevent multiple definition of the Student
              // Records structure, enabling contents of cms.h to be copied to
              // main.c and operations.c once
#define CMS_H
...
newColumn
    custom_column[MAX_CUSTOM_COLUMN_NO]; // Array to store all the new columns
...
const char *default_column_names[NUM_DEFAULT_COLS] = {"ID", "Name", "Programme",
                                                      "Mark"};
...
void update_operation(char *command);
void delete_operation(char *command);
void save_operation(const char *filename, const StudentRecords *db, int count,
                    newColumn custom_column[], int num_custom_cols);
void sort_operation();
...
int compIDAscend(
    const void *a,
    const void *b); // Custom function to sort the ID (Ascending Order)
int compIDDescend(
    const void *a,
    const void *b); // Custom function to sort the ID (Descending Order)
int compMarksAscend(
    const void *a,
    const void *b); // Custom function to sort the Marks (Ascending Order)
int compMarksDescend(
    const void *a,
    const void *b); // Custom function to sort the Marks (Descending Order)
...
int checkColumnNameExists(const char *colName, newColumn custom_column[],
                          int num_custom_cols); // For add column operation
